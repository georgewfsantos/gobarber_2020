...
interface AvailableTimeTextProps {
  selected: boolean;
}
...
export const AvailableTime = styled(RectButton)<AvailableTimeProps>`
  padding: 12px;
  background: ${(props) => (props.selected ? '#ff9000' : '#3e3b47')};
  border-radius: 10px;
  margin-right: 8px;
...
export const AvailableTimeText = styled.Text<AvailableTimeTextProps>`
  color: ${(props) => (props.selected ? '#232129' : '#f4ede8')};
  font-family: 'RobotoSlab-Regular';
  font-size: 16px;
`;
